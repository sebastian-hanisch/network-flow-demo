...
def generate_distribution_plan_pdf(instance, result_label, flow, cost, runtime_ms):
    ...
    pdf.ln(7)
    pdf.set_font("Helvetica", "", 9)
    relevant_kinds = {"transport_werk_dc", "transport_dc_filiale", "fehlmenge"}
    for a in instance.arcs:
        f = flow.get(a.idx, 0.0)
        if a.kind not in relevant_kinds or f <= 1e-6:
            continue
        von = a.tail.replace("_in", "").replace("_out", "")
        nach = a.head.replace("_in", "").replace("_out", "")
        if a.kind == "fehlmenge":
            von = "Notbeschaffung"
        row = [von, nach, f"{f:.1f}", f"{a.cost:.2f}"]
        for val, w in zip(row, widths):
            pdf.cell(w, 6, val, border=1, new_x=XPos.RIGHT, new_y=YPos.TOP)
        pdf.ln(6)
